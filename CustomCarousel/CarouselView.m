...
- (NSUInteger)numberOfPlaceholdersInCarousel:(__unused CarouselView *)carousel { return 0; }
- (void)carouselWillBeginScrollingAnimation:(__unused CarouselView *)carousel {}
- (void)carouselDidEndScrollingAnimation:(__unused CarouselView *)carousel {}
- (void)carouselDidScroll:(__unused CarouselView *)carousel {}

- (void)carouselCurrentItemIndexDidChange:(__unused CarouselView *)carousel {}
- (void)carouselWillBeginDragging:(__unused CarouselView *)carousel {}
- (void)carouselDidEndDragging:(__unused CarouselView *)carousel willDecelerate:(__unused BOOL)decelerate {}
- (void)carouselWillBeginDecelerating:(__unused CarouselView *)carousel {}
- (void)carouselDidEndDecelerating:(__unused CarouselView *)carousel {}

- (BOOL)carousel:(__unused CarouselView *)carousel shouldSelectItemAtIndex:(__unused NSInteger)index { return YES; }
- (void)carousel:(__unused CarouselView *)carousel didSelectItemAtIndex:(__unused NSInteger)index {}

- (CGFloat)carouselItemWidth:(__unused CarouselView *)carousel { return 0; }
- (CATransform3D)carousel:(__unused CarouselView *)carousel
   itemTransformForOffset:(__unused CGFloat)offset
            baseTransform:(CATransform3D)transform { return transform; }
- (CGFloat)carousel:(__unused CarouselView *)carousel
...
NSComparisonResult compareViewDepth(UIView *view1, UIView *view2, CarouselView *self);
...
- (CATransform3D)transformForItemViewWithOffset:(CGFloat)offset
{
    ...
    switch (_type)
    {
        ...
        case iCarouselTypeInvertedWheel:
        {
            ...
            CGFloat xxOther = -50;
            CGFloat xxx = -10;
            CGFloat xMaxScale = 1.0;
            CGFloat xMinScale = 1.0;
            if (_type == iCarouselTypeInvertedWheel)
            {
                radius = -radius;
                angle = -angle;
            ...
            if (_vertical)
            {
                ...
                transform = CATransform3DRotate(transform, angle * offset, 0.0, 0.0, 1.0);
                //旋转后x位移
                transform = CATransform3DTranslate(transform, radius+xxx, 0.0, offset * 0.01);
                //再旋转回来
                transform = CATransform3DRotate(transform, -angle * offset, 0.0, 0.0, 1.0);
                //x位移
                if (angle*offset == 0) {
                    transform = CATransform3DTranslate(transform, xx, 0.0, 0.0);
                    transform = CATransform3DScale(transform, xMaxScale, xMaxScale, 1);
                }else {
                    transform = CATransform3DTranslate(transform, xxOther, 0.0, 0.0);
                    transform = CATransform3DScale(transform, xMinScale, xMinScale, 1);
                }
                return transform;
            }
            else
...
- (void)addCenterImage{
    //    bgImageView1 = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"sy_zhuanpan"]];
    //    [self addSubview:bgImageView1];
    //    [self sendSubviewToBack:bgImageView1];
    //    bgImageView1.frame = CGRectMake(SCREEN_WIDTH/2, 0, 307 * DO_HEIGHT_RATIO_HOME, 307 * DO_HEIGHT_RATIO_HOME);
    //
    //    centerImageBg = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"sy_jb_l"]];
    //    [self addSubview:centerImageBg];
    //    centerImageBg.frame = CGRectMake(SCREEN_WIDTH-60+6, 307 * DO_HEIGHT_RATIO_HOME/2-28, 60, 60);
    //
    //    centerImage = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"sy_jingbao_icon"]];
    //    [self addSubview:centerImage];
    //    centerImage.frame = CGRectMake(SCREEN_WIDTH-48, 307 * DO_HEIGHT_RATIO_HOME/2-24, 48, 48);
}

- (void)changeLeftStyle:(BOOL)left {
    //    if (left) {
    //        bgImageView1.left = -SCREEN_WIDTH/2+(SCREEN_WIDTH- DO_HEIGHT_RATIO_HOME*307);
    //        centerImageBg.left = -6;
    //        centerImage.left = 0;
    //
    //    }else {
    //        bgImageView1.left = SCREEN_WIDTH/2;
    //        centerImageBg.left = self.width-60+6;
    //        centerImage.left = self.width-48;
    //    }
}
